...
public class _6_01_Knapsack {
    ...
    public static int maxProfit_recursion(int wt[], int val[], int n, int W) {
        ...
        if (wt[n - 1] <= W) {
            int take = val[n - 1] + maxProfit_recursion(wt, val, n - 1, W - wt[n - 1]);
    ...
    public static int maxProfit_memoization_util(int wt[], int val[], int n, int W, int dp[][]) {
        ...
        if (wt[n - 1] <= W) {
            int take = val[n - 1] + maxProfit_memoization_util(wt, val, n - 1, W - wt[n - 1], dp);
    ...
    public static int maxProfit_spaceOptimization(int wt[], int val[], int n, int W) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            for (int j = 1; j <= W; j++) {
                if (wt[i - 1] <= j) {
                    int take = val[i - 1] + prev[j - wt[i - 1]];
    ...
    // Most Optimized - O(N*W), O(W)
    public static int maxProfit_mostOptimized(int wt[], int val[], int n, int W) {
        int prev[] = new int[W + 1];
